...
export const authController = {
    ...
    async submitRegistration(req, res) {
        ...
        const effectiveGoogleUid = payload.googleUid ||
            payload.userId ||
     ...
     */
    async getPendingForReferrer(req, res) {
        ...
        const isGlobalAdmin = accountId === 'admin_miftah_99' ||
     ...
     */
    async appApproveRegistration(req, res) {
     ...
     */
    async appRejectRegistration(req, res) {
        const id = getParam(req.params.id);
        const updatedReg = await prisma.alumniRegistration.update({
            where: { id },
            data: {
     ...
     */
    async resendReferralEmail(req, res) {
    ...
    async verifyEmailAction(req, res) {
        ...
        try {
            ...
            if (!reg) {
                res.status(404).send(renderResponseHtml({
