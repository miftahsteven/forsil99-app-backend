...
export function formatPostResponse(post, currentUserId) {
    ...
    const userReactionObj = currentUserId
        ...
        : undefined;
    const reactors = (post.reactions || []).map((r) => ({
        uid: r.userId,
    ...
    return {
        ...
        shopCategory: post.memoryMeta?.shopCategory || undefined,
        price: post.memoryMeta?.price || undefined,
...
export const postController = {
        ...
        const visiblePosts = posts.filter((p) => {
            if (p.visibility === 'same_class') {
                ...
                if (p.authorId === currentUserId)
                    return true;
                if (currentUserClass && postAuthorClass && currentUserClass === postAuthorClass)
                    return true;
                return false;
            }
            return true;
    ...
    async createPost(req, res) {
        ...
        const { type = 'standard', text = '', media = [], visibility = 'verified_alumni', memoryMeta, shopCategory, price, linkedProductId, linkedEventId, commentsEnabled = true, } = req.body;
    ...
    async reactToPost(req, res) {
        ...
        if (existingReaction) {
            ...
            if (post.authorId !== userId) {
                ...
                const reactionLabel = reactionType === 'kangen' ? 'Kangen' : reactionType === 'salut' ? 'Salut' : 'Suka';
