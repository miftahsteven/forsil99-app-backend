...
export const shopController = {
  ...
  async getProductById(req: Request, res: Response): Promise<void> {
    ...
    if (!p) {
      ...
      if (post && post.type === 'shop_share') {
        const mediaUrls = Array.isArray(post.media) ? (post.media as any[]).map((m) => m.url).filter(Boolean) : [];
        ...
        res.json({
          ...
          product: {
            ...
            type: 'product',
            categoryId: (post.memoryMeta as any)?.shopCategory || 'lainnya',
            categoryName: 'Aneka Kebutuhan',
            description: post.text,
            imageUrls: mediaUrls,
            priceType: (post.memoryMeta as any)?.price ? 'fixed' : 'contact_seller',
            price: (post.memoryMeta as any)?.price ? Number((post.memoryMeta as any).price) : undefined,
            status: 'active',
  ...
  async createProduct(req: Request, res: Response): Promise<void> {
    ...
    const {
      ...
      city,
      serviceAreas = [],
      durationDays = 30, // Default 30 days active
    } = req.body;
    ...
    try {
      const mediaList = (Array.isArray(imageUrls) ? imageUrls : []).map((url: string) => ({
        type: 'image',
        url,
    ...
    if (!existing) {
      ...
      if (existingPost) {
        ...
        const media = Array.isArray(imageUrls)
          ? imageUrls.map((u: string) => ({ type: 'image', url: u, caption: (name || '').slice(0, 30) }))
          : existingPost.media;
