 ...
 */
export async function getDeceasedAlumni(req: Request, res: Response): Promise<void> {
  try {
    ...
    const result = records.map((item: any) => {
      const userFlower = currentUserId
        ? item.flowers.find((f: any) => f.userId === currentUserId)
        : null;
      ...
      return {
        ...
        recentPrayers: item.prayers.map((p: any) => ({
          id: p.id,
 ...
 */
export async function getDeceasedAlumniById(req: Request, res: Response): Promise<void> {
  try {
    const id = String(req.params.id);
    const currentUserId = (req as any).user?.id as string | undefined;
    const now = new Date();

    const item: any = await prisma.deceasedAlumni.findUnique({
      where: { id },
      include: {
        flowers: {
    ...
    res.json({
      ...
      data: {
        ...
        hasGivenFlower: Boolean(userFlower),
        flowerExpiresAt: userFlower ? userFlower.expiresAt.toISOString() : null,
        prayers: item.prayers.map((p: any) => ({
          id: p.id,
          deceasedId: p.deceasedId,
          authorId: p.authorId,
          authorName: p.author?.profile?.fullName || p.author?.email || 'Alumni 99',
 ...
 */
export async function giveFlower(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (existingFlower) {
      const daysRemaining = Math.max(
        1,
        Math.ceil((existingFlower.expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
      );
      res.status(400).json({
        success: false,
 ...
 */
export async function getPrayers(req: Request, res: Response): Promise<void> {
  try {
    const deceasedId = String(req.params.id);
    ...
    const formatted = prayers.map((p: any) => ({
      id: p.id,
      deceasedId: p.deceasedId,
      authorId: p.authorId,
      authorName: p.author?.profile?.fullName || p.author?.email || 'Alumni 99',
 ...
 */
export async function submitPrayer(req: Request, res: Response): Promise<void> {
  try {
    const deceasedId = String(req.params.id);
    const userId = (req as any).user?.id as string | undefined;
    const { text } = req.body;
    ...
    const prayer: any = await prisma.memorialPrayer.create({
      data: {
        deceasedId,
        authorId: userId,
        text: text.trim(),
 ...
 */
export async function createDeceasedAlumni(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.id as string | undefined;
 ...
 */
export async function deleteDeceasedAlumni(req: Request, res: Response): Promise<void> {
  try {
    const id = String(req.params.id);
    const userId = (req as any).user?.id as string | undefined;
    const userRoles: string[] = (req as any).user?.roles || [];
